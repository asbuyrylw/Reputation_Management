...
|---|---|---|
| **PostgreSQL database** | `REP_DB_DSN` | A Postgres 14+ instance. Local dev uses the `rep_pg_test` Docker container on `:15432`. Production: any managed Postgres (RDS, Supabase, Neon, etc.). Run `alembic upgrade head` to create the schema (currently head `0059`). |
