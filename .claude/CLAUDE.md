## vexp - Context-Aware AI Coding <!-- vexp v2.1.6 -->
...
<!-- context-tool policy (hand-maintained; lives OUTSIDE the vexp block so regeneration won't clobber it) -->
## Code-context tools: vexp is primary here

vexp (block above) is the primary code-context engine for this repo — `run_pipeline`
first, `get_skeleton` over Read. The **code-review-graph** MCP is *complementary, not
a duplicate*: reach for it only for code **review** workflows (`detect_changes`,
`get_review_context`, `get_impact_radius`, `get_affected_flows`) and architecture/wiki
(`generate_wiki`, `get_architecture_overview`, `get_knowledge_gaps`). Do not run both
as "first" for the same task: **vexp for building/understanding, code-review-graph for
reviewing changes.**