...
const sevCls = (s: string) =>
  s === "critical" ? "border-rose-200 bg-rose-50" : s === "warning" ? "border-amber-200 bg-amber-50" : "border-gray-200 bg-white";
      ...
      {data.items.length === 0 ? (
        ...
        <>
          {data.unread > 0 && canEdit && (
            <div className="mb-3">
              <button onClick={() => readAll.mutate()} className="text-sm font-medium text-blue-600 hover:underline">
              ...
              <Card key={n.id} className={`${sevCls(n.severity)} ${n.read ? "opacity-60" : ""}`}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    {n.body && <div className="mt-0.5 text-sm text-gray-600">{n.body}</div>}
                    {n.created_at && <div className="mt-1 text-xs text-gray-400">{new Date(n.created_at).toLocaleString()}</div>}
                  </div>
                  {!n.read && canEdit && (
                    <button onClick={() => read.mutate({ id: n.id })} className="shrink-0 text-xs font-medium text-blue-600 hover:underline">
                      Mark read
                    </button>
                  )}
                </div>
