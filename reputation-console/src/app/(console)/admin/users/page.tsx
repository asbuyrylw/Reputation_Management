...
function GrantRow({ userId, businesses, grant }: { userId: number; businesses: Business[]; grant: GrantMutation }) {
  ...
  return (
    <div className="mt-1 flex flex-wrap items-center gap-2">
      <select value={bid} onChange={(e) => setBid(Number(e.target.value))} className="rounded border border-gray-200 px-2 py-1 text-xs">
      ...
      <select value={ar} onChange={(e) => setAr(e.target.value)} className="rounded border border-gray-200 px-2 py-1 text-xs">
...
export default function UsersPage() {
  ...
  return (
    <div>
      ...
      />
      <Card className="mb-4">
        <div className="mb-2 text-sm font-medium text-gray-700">Add user</div>
        <div className="flex flex-wrap items-end gap-2">
          <input placeholder="email" value={email} onChange={(e) => setEmail(e.target.value)} className="rounded-md border border-gray-300 px-3 py-1.5 text-sm" />
          <input placeholder="password (8+ chars)" type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="rounded-md border border-gray-300 px-3 py-1.5 text-sm" />
