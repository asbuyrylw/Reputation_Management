...
import { useContentDrafts, useAssets, useWorkOrders, useGenerateDraftForWo } from "@/lib/hooks";
...
type Piece = {
  ...
  woId: number | null;      // originating work order (enables Regenerate)
  title: string;
  ...
  keywords: string;
  href: string;
  // Content scores, read from the piece's quality_notes (published pieces borrow their draft's).
  geo: number | null;       // AI-citability grade
  aeo: number | null;       // answer-engine readiness
  seo: number | null;       // on-page SEO
  kw: number | null;        // target-keyword coverage %
  natural: number | null;   // distinctiveness (reads human, not AI)
};
...
// Pull the 0-100 scores off a draft's quality_notes (assets don't carry it — they borrow the
// originating draft's via the work order). keyword_coverage.rate is 0..1, shown as a %.
function scoresFrom(qn: ContentDraft["quality_notes"] | undefined): Pick<Piece, "geo" | "aeo" | "seo" | "kw" | "natural"> {
  return {
    geo: qn?.geo?.score ?? null,
    aeo: qn?.aeo?.score ?? null,
    seo: qn?.on_page?.score ?? null,
    kw: qn?.keyword_coverage?.rate != null ? Math.round(qn.keyword_coverage.rate * 100) : null,
    natural: qn?.distinctiveness?.score ?? null,
  };
}
// A right-aligned 0-100 score column (color-graded, sortable, hidden by default to keep the grid calm).
function scoreCol(key: string, label: string, get: (p: Piece) => number | null, defaultHidden = false): DataGridColumn<Piece> {
  return {
    key, label, width: 62, align: "right", defaultHidden,
    sortValue: (p) => get(p) ?? -1,
    render: (p) => { const n = get(p); return <span className={`font-mono text-xs font-semibold ${scoreCls(n)}`}>{n ?? "—"}</span>; },
  };
}

// The Regenerate action — one click makes a fresh AI version of this piece (evergreen / recycle),
// landing as a new draft in the review queue. A component so each row owns its own mutation state.
function RegenCell({ woId, businessId }: { woId: number | null; businessId: number | null }) {
  const gen = useGenerateDraftForWo(businessId);
  if (woId == null) return <span className="text-slate-300">—</span>;
  return (
    <button
      type="button"
      onClick={(e) => { e.stopPropagation(); gen.mutate({ woId }); }}
      disabled={gen.isPending || gen.isSuccess}
      title="Generate a fresh version of this piece (evergreen) — lands as a new draft to review"
      className="rounded-md border border-indigo-200 px-2 py-0.5 text-[11px] font-semibold text-indigo-600 hover:bg-indigo-50 disabled:opacity-50"
    >
      {gen.isPending ? "…" : gen.isSuccess ? "Queued ✓" : "↻ Regenerate"}
    </button>
  );
}
...
const COLUMNS: DataGridColumn<Piece>[] = [
  ...
  scoreCol("geo", "GEO", (p) => p.geo),
  scoreCol("aeo", "AEO", (p) => p.aeo),
  scoreCol("seo", "SEO", (p) => p.seo),
  scoreCol("kw", "Keywords", (p) => p.kw, true),
  scoreCol("natural", "Natural", (p) => p.natural, true),
  { key: "keywords", label: "Target query", width: 200, defaultHidden: true, sortValue: (p) => p.keywords.toLowerCase(), render: (p) => <span className="text-xs text-slate-500">{p.keywords || "—"}</span> },
...
export default function LibraryPage() {
  ...
  const pieces = useMemo<Piece[]>(() => {
    ...
    // Scores live on the DRAFT's quality_notes; a published asset borrows them via its work order.
    const notesByWo = new Map<number, ContentDraft["quality_notes"]>();
    for (const d of drafts ?? []) if (d.work_order_id != null && d.quality_notes && !notesByWo.has(d.work_order_id)) notesByWo.set(d.work_order_id, d.quality_notes);
    const out: Piece[] = [];
    ...
    for (const a of assets ?? []) {
      ...
      out.push({
        key: `a${a.id}`, woId: a.work_order_id ?? null, title: a.title ?? "Untitled", type, gap, section: sectionOf(gap),
        ...
        ...scoresFrom(a.work_order_id != null ? notesByWo.get(a.work_order_id) : undefined),
      });
    }
    for (const d of drafts ?? []) {
      ...
      out.push({
        key: `d${d.id}`, woId: d.work_order_id ?? null, title: d.title ?? "Untitled", type, gap, section: sectionOf(gap),
        area: wo?.area ? wo.area.charAt(0).toUpperCase() + wo.area.slice(1) : areaOf(type),
        status: draftStatusLabel[d.status] ?? "Draft",
        words: wordCount(d.body), keywords: d.target_query ?? "", href: "/content/drafts",
        ...scoresFrom(d.quality_notes),
      });
    }
    return out;
  }, [drafts, assets, workOrders]);

  // Editors get a Regenerate action column (evergreen — one click makes a fresh version).
  const columns = useMemo<DataGridColumn<Piece>[]>(() => (
    canEdit
      ? [...COLUMNS, {
          key: "regen", label: "Refresh", width: 108, hideable: false,
          render: (p: Piece) => <RegenCell woId={p.woId} businessId={businessId} />,
        }]
      : COLUMNS
  ), [canEdit, businessId]);
