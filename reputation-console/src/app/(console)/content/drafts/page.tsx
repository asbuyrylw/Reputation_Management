  ...
  return (
    <div>
      ...
      <div className="mb-4 flex items-center gap-4 text-sm">
        <button onClick={() => setShowAll(false)} className={!showAll ? "font-medium text-gray-900" : "text-gray-500"}>
          Pending ({pending.length})
        </button>
        <button onClick={() => setShowAll(true)} className={showAll ? "font-medium text-gray-900" : "text-gray-500"}>
          All ({data.length})
        </button>
      </div>
      {shown.length === 0 ? (
