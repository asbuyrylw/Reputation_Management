...
const band = (s: number) =>
  s >= 70 ? { t: "text-emerald-700", b: "bg-emerald-50 border-emerald-200", word: "Good" }
  : s >= 40 ? { t: "text-amber-700", b: "bg-amber-50 border-amber-200", word: "Needs work" }
  : { t: "text-rose-700", b: "bg-rose-50 border-rose-200", word: "Poor" };

// ---- tiny inline icons ----
function Check() {
  return (
    <svg viewBox="0 0 20 20" className="h-3.5 w-3.5 shrink-0 text-emerald-600" fill="currentColor" aria-hidden>
...
function X() {
  return (
    <svg viewBox="0 0 20 20" className="h-3.5 w-3.5 shrink-0 text-rose-500" fill="currentColor" aria-hidden>
...
export function SiteAuditView({ summary }: { summary: Record<string, unknown> }) {
  ...
  // "only N pages" message reads as an honest site-structure finding, not a crawler failure.
  const artifacts = Math.max(0, (s.pages_crawled ?? allPages.length) - pages.length);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`rounded-xl border p-4 ${overall == null ? "border-slate-200 bg-slate-50" : band(overall).b}`}>
        <p className={`text-sm font-medium ${overall == null ? "text-slate-600" : band(overall).t}`}>{verdict}</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Stat
          ...
          value={
            <span className={overall === null ? "text-slate-400" : band(overall).t}>
        ...
        <Stat label="Pages needing work" value={<span className={needWork ? "text-amber-700" : "text-emerald-700"}>{needWork}</span>} sub="have ≥1 issue" />
        <Stat
          label={<Term name="schema">Schema coverage</Term>}
          value={<span className={schemaPresent.length ? "text-emerald-700" : "text-rose-600"}>{schemaPresent.length}/{schemaPresent.length + schemaGaps.length}</span>}
      ...
      {pages.length > 0 && pages.length <= 2 && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5 text-xs text-amber-800">
          <div className="font-semibold">
            Your site has only {pages.length} real content {pages.length === 1 ? "page" : "pages"}
            {artifacts > 0 ? ` — the other ${artifacts} ${artifacts === 1 ? "URL was" : "URLs were"} WordPress system links (feeds, APIs, code), not real pages.` : "."}
          </div>
          <p className="mt-1">
            That&apos;s itself a gap, not just a crawl limit: with little of your own material to quote, AI leans on
            outside sources you don&apos;t control. Adding real pages (About, Services, Team, FAQs) is one of the
            highest-impact fixes — your content plan turns these into tasks.
          </p>
          <Link href="/content/work-orders" className="mt-1.5 inline-block font-semibold text-amber-900 underline hover:text-amber-950">
            See the pages to create →
          </Link>
        </div>
      )}
      ...
      {issues.length > 0 && (
        <div>
          <h2 className="mb-1 text-base font-semibold text-slate-900">Top fixes across the site</h2>
          <p className="mb-3 text-sm text-slate-500">Ordered by how many pages are affected.</p>
          <div className="overflow-hidden rounded-lg border border-slate-200">
            {issues.map(([code, count], i) => (
              <div key={code} className={`flex items-start gap-3 px-4 py-3 ${i ? "border-t border-slate-100" : ""}`}>
                <span className={`mt-0.5 inline-block h-2.5 w-2.5 shrink-0 rounded-full ${issueSev(code) === "high" ? "bg-rose-500" : "bg-amber-400"}`} />
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-slate-900">{issueLabel(code)}</span>
                    <span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600">{count} {count === 1 ? "page" : "pages"}</span>
                  </div>
                  <div className="text-sm text-slate-500">{issueFix(code)}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="rounded-lg border border-slate-200 p-4">
          <h3 className="mb-2 text-sm font-semibold text-slate-900">Structured data (schema)</h3>
          {schemaPresent.length > 0 && (
            <div className="mb-2 flex flex-wrap gap-1.5">
              {schemaPresent.map((t) => (
                <span key={t} className="inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-xs text-emerald-700"><Check />{t}</span>
              ))}
            </div>
          )}
          <div className="flex flex-wrap gap-1.5">
            {schemaGaps.map((t) => (
              <span key={t} className="inline-flex items-center gap-1 rounded-full border border-rose-200 bg-rose-50 px-2 py-0.5 text-xs text-rose-700"><X />{t}</span>
            ))}
          </div>
          {schemaGaps.length > 0 && <p className="mt-2 text-xs text-slate-500">Add these JSON-LD types so AI + search engines can read the business clearly.</p>}
        </div>
        <div className="rounded-lg border border-slate-200 p-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {(s.top_missing_entities ?? []).map((e) => (
              <span key={e} className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-xs text-slate-700">{e}</span>
            ))}
          </div>
      ...
      {/* ---- per-page breakdown ---- */}
      <div>
        <h2 className="mb-1 text-base font-semibold text-slate-900">Page-by-page</h2>
        <p className="mb-3 text-sm text-slate-500">Lowest-scoring pages first — click a page for the fixes.</p>
        <div className="space-y-2">
          {pages.map((p) => {
            ...
            return (
              <div key={p.url} className="overflow-hidden rounded-lg border border-slate-200">
                <button
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-sm font-medium text-slate-900">{pathOf(p.url)}</div>
                    <div className="truncate text-xs text-slate-400">{p.title || "(no title)"} · {p.word_count ?? 0} words</div>
                  </div>
                  <span className={`hidden shrink-0 rounded-full border px-2 py-0.5 text-xs font-medium sm:inline ${c.b} ${c.t}`}>{c.word}</span>
                  {(p.issues?.length ?? 0) > 0 && (
                    <span className="shrink-0 rounded-full bg-rose-50 px-2 py-0.5 text-xs font-medium text-rose-600">{p.issues!.length} {p.issues!.length === 1 ? "issue" : "issues"}</span>
                  )}
                  <svg viewBox="0 0 20 20" className={`h-4 w-4 shrink-0 text-slate-400 transition-transform ${isOpen ? "rotate-180" : ""}`} fill="currentColor" aria-hidden><path d="M5.2 7.5L10 12.3l4.8-4.8 1.2 1.2-6 6-6-6z" /></svg>
                </button>
                {isOpen && (
                  <div className="border-t border-slate-100 bg-slate-50/60 px-4 py-3">
                    <a href={p.url} target="_blank" rel="noreferrer" className="mb-3 inline-block break-all text-xs text-indigo-600 hover:underline">{p.url}</a>
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div>
                        <div className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-slate-400">Working well</div>
                        {good.length ? (
                          <ul className="space-y-1">
                            {good.map((g) => (<li key={g} className="flex items-center gap-1.5 text-sm text-slate-700"><Check />{g}</li>))}
                          </ul>
                        ) : <p className="text-sm text-slate-400">—</p>}
                      </div>
                      <div>
                        <div className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-slate-400">Fix these</div>
                        {(p.issues?.length ?? 0) ? (
                          <ul className="space-y-2">
                            {p.issues!.map((iss) => (
                              <li key={iss} className="flex items-start gap-1.5 text-sm">
                                <X />
                                <span><span className="font-medium text-slate-800">{issueLabel(iss)}.</span> <span className="text-slate-500">{issueFix(iss)}</span></span>
                              </li>
                            ))}
                          </ul>
                        ) : <p className="flex items-center gap-1.5 text-sm text-emerald-700"><Check />No issues found.</p>}
                      </div>
                    </div>
                  </div>
                )}
          ...
          {pages.length === 0 && <p className="text-sm text-slate-500">No content pages found in the crawl.</p>}
        </div>
      </div>
    </div>
  );
