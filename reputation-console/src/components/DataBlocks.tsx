...
function Value({ value }: { value: unknown }) {
  if (Array.isArray(value)) {
    if (value.length === 0) return <p className="text-sm text-slate-400">—</p>;
    return (
  ...
  return <p className="mt-0.5 text-sm text-slate-700">{String(value)}</p>;
}
...
function Block({ label, value }: { label: string; value: unknown }) {
  ...
  return (
    <div>
      <div className="text-sm font-semibold text-slate-800">{humanize(label)}</div>
      <Value value={value} />
    </div>
  );
}
