  ...
  return (
    <Card className="border-blue-200 bg-blue-50/40">
      ...
      <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-blue-100">
        <div className="h-full rounded-full bg-blue-500" style={{ width: `${(data.done / data.total) * 100}%` }} />
      </div>
      <ul className="mt-3 space-y-1.5">
        {data.steps.map((s) => (
          <li key={s.key} className="flex items-center justify-between gap-3 text-sm">
            <span className={s.done ? "text-gray-400 line-through" : "text-gray-800"}>
              <span className="mr-1.5">{s.done ? "✓" : "○"}</span>{s.label}
            </span>
            {!s.done && (
              <Link href={s.href} className="shrink-0 text-xs font-medium text-blue-600 hover:underline">
                Do this →
              </Link>
            )}
          </li>
