...
export function PerEnginePanel({ data }: { data: PerEngineMetrics }) {
  ...
  return (
    <Card>
      <h2 className="mb-1 text-lg font-semibold text-slate-900">What each AI engine says</h2>
      ...
      {data.coverage.partial && (
        <div className="mb-3 rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-800">
          Partial coverage — {data.coverage.configured.join(", ") || "no engines"} (
          {data.coverage.configured.length} of {data.coverage.expected.length} AI engines) ran.
            ...
            {engines.map(([name, m]) => {
              ...
              return (
                <tr key={name} className="border-b last:border-0">
                  <td className="py-2 pr-4 font-medium text-slate-900">{engineLabel(name)}</td>
                  <td className="py-2 pr-4 text-slate-600">{m.n}</td>
                    ...
                    {ga?.mean == null ? (
                      ...
                      <span className="inline-flex items-center gap-1.5">
                        ...
                        {ga.low != null && ga.high != null && (
                          <span className="text-xs text-slate-400">CI {repScore(ga.low)}–{repScore(ga.high)}</span>
                        )}
                      </span>
                    )}
                  </td>
